...
export default function MyPoolsPage() {
      ...
      try {
        ...
        if (accounts.length > 0) {
          ...
          if (userRole !== "provider") {
            toast.error("Only scholarship providers can access this page");
            router.push("/Home");
          }
        } else {
          router.push("/");
  ...
  const fetchPools = async () => {
    try {
      ...
      if (POOL_FACTORY_ADDRESS === "0x0000000000000000000000000000000000000000") {
        toast.error("PoolFactory contract not deployed. Please deploy contracts first.");
        setLoading(false);
        return;
      }
      ...
      try {
        while (true) {
          const poolAddress = await poolFactory.poolsByCreator(walletAddress, index);
  ...
  const handleFundPool = async (poolAddress: string) => {
    ...
    try {
      ...
      await tx.wait();
      toast.success(`Pool funded with ${amount} ETH!`, { id: loadingToast });
  ...
  const handlePausePool = async (poolAddress: string, isPaused: boolean) => {
    try {
      const loadingToast = toast.loading(isPaused ? "Unpausing pool..." : "Pausing pool...");
      ...
      const tx = isPaused ? await poolContract.unpause() : await poolContract.pause();
      await tx.wait();

      toast.success(isPaused ? "Pool unpaused!" : "Pool paused!", { id: loadingToast });
      fetchPools(); // Refresh data
    } catch (error: any) {
  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <main className="container mx-auto px-8 py-12">
        {/* Page Title */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-white mb-2">My Scholarship Pools</h1>
          <p className="text-gray-400">Manage your scholarship pools and review applications</p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            ...
            <div className="p-6 bg-white/5 backdrop-blur-md rounded-xl border border-white/10">
              ...
              <p className="text-3xl font-bold text-blue-400">
                {pools.reduce((sum, p) => sum + p.applicantCount, 0)}
              </p>
            </div>
            <div className="p-6 bg-white/5 backdrop-blur-md rounded-xl border border-white/10">
              <p className="text-gray-400 text-sm mb-1">Scholarships Awarded</p>
              <p className="text-3xl font-bold text-green-400">
                {pools.reduce((sum, p) => sum + p.totalScholarshipsAwarded, 0)}
              </p>
            </div>
            <div className="p-6 bg-white/5 backdrop-blur-md rounded-xl border border-white/10">
              <p className="text-gray-400 text-sm mb-1">Total Funds</p>
              <p className="text-3xl font-bold text-purple-400">
                {pools.reduce((sum, p) => sum + parseFloat(p.totalFunds), 0).toFixed(2)} ETH
              </p>
            </div>
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            <div className="max-w-md mx-auto space-y-4">
              ...
              <h3 className="text-2xl font-bold text-white">No Pools Yet</h3>
              <p className="text-gray-400">
                You haven't created any scholarship pools. Create your first pool to start helping students!
              </p>
            ...
            {pools.map((pool) => (
              ...
              >
                ...
                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="text-2xl font-bold text-white">{pool.poolName}</h3>
                      {pool.isPaused && (
                    ...
                    <p className="text-gray-400 text-sm mb-2">{pool.poolDescription}</p>
                    <p className="text-gray-500 text-xs">Contract: {truncateAddress(pool.address)}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-4">
                  <div>
                    <p className="text-xs text-gray-500">Scholarship Amount</p>
                    <p className="text-white font-semibold">{pool.scholarshipAmount} ETH</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500">Available Funds</p>
                    <p className="text-purple-400 font-semibold">{pool.availableFunds} ETH</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500">Total Funds</p>
                    <p className="text-white font-semibold">{pool.totalFunds} ETH</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500">Applications</p>
                    <p className="text-blue-400 font-semibold">{pool.applicantCount}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500">Deadline</p>
                    <p className="text-white font-semibold">{formatDate(pool.applicationDeadline)}</p>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-3">
                  <button
                    onClick={() => fetchPoolApplications(pool.address)}
                  ...
                  <button
                    onClick={() => handleFundPool(pool.address)}
                    className="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-lg border border-white/20 text-white transition-all"
                  >
                    💰 Add Funds
                  </button>
                  <button
                    onClick={() => handlePausePool(pool.address, pool.isPaused)}
                    className="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-lg border border-white/20 text-white transition-all"
                  >
                    {pool.isPaused ? "▶️ Resume" : "⏸️ Pause"}
                  </button>
                  <a
                    ...
                    {loadingApplications ? (
                      ...
                      <div className="space-y-3">
                        {poolApplications.map((app) => (
                          ...
                          >
                            <div className="flex-1">
                              <p className="text-white font-semibold">{app.applicationData.name}</p>
                              <p className="text-gray-400 text-sm">{app.applicationData.institution} - {app.applicationData.program}</p>
                              <p className="text-gray-500 text-xs">{truncateAddress(app.walletAddress)}</p>
                            </div>
                            <div className="flex items-center gap-3">
                              {app.isPaid ? (
                                <span className="px-3 py-1 rounded-full bg-purple-500/20 text-purple-300 text-xs font-semibold">Paid</span>
                              ) : app.adminApproved ? (
                                <span className="px-3 py-1 rounded-full bg-green-500/20 text-green-300 text-xs font-semibold">Approved</span>
                              ) : app.emailVerified ? (
                                <span className="px-3 py-1 rounded-full bg-blue-500/20 text-blue-300 text-xs font-semibold">Under Review</span>
                              ) : (
                                <span className="px-3 py-1 rounded-full bg-yellow-500/20 text-yellow-300 text-xs font-semibold">Pending</span>
                              )}
                            </div>
                          </div>
                        ))}
