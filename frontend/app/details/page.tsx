...
export default function DetailsPage() {
  ...
  const [pendingRole, setPendingRole] = useState<"student" | "provider" | null>(null);
  ...
  const connectWallet = async () => {
    ...
    try {
      ...
      toast.loading("Checking wallet registration...", { id: loadingToast });
      ...
      try {
        ...
        if (data.registered && data.user) {
          ...
          toast.success("Welcome back! Redirecting to Home...", { id: loadingToast });
          setTimeout(() => {
            router.push("/Home");
          }, 1000);
          return; // Keep loading state until redirect
  ...
  const handleProviderInput = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    setProviderForm({ ...providerForm, [e.target.name]: e.target.value });
  };

  // Submit student registration
  const submitStudent = async () => {
    if (!studentForm.fullName || !studentForm.email || !studentForm.institute || !studentForm.program || !studentForm.graduationYear) {
    ...
    try {
      ...
      // Call backend API
      const res = await fetch("http://localhost:5000/api/onboarding/student", {
        method: "POST",
        body: formData,
      });
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.message || "Failed to submit student registration");
      }
      ...
      toast.success("Registration submitted! Check your email for verification.", { id: loadingToast });
    } catch (e: any) {
      console.error(e);
  ...
  const submitProvider = async () => {
    if (!providerForm.organizationName || !providerForm.email || !providerForm.description || !providerForm.contactPerson) {
      toast.error("Please fill all required fields.");
      return;
    }
    if (!walletAddress) {
    ...
    try {
      ...
      // Call backend API
      const res = await fetch("http://localhost:5000/api/onboarding/provider", {
        method: "POST",
        body: formData,
      });
      ...
      toast.success("Provider registration submitted! Check your email for verification.", { id: loadingToast });
    } catch (e: any) {
      console.error(e);
      toast.error(e.message || "Submission failed. Please try again.");
    } finally {
    ...
    <div className="min-h-screen bg-black overflow-x-hidden flex flex-col">
      ...
      {checkingWallet && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm">
          <div className="text-center">
            ...
            <h3 className="text-xl font-semibold text-white mb-2">Checking Wallet Registration</h3>
            <p className="text-gray-400 text-sm">Verifying if your wallet is already registered...</p>
          </div>
        </div>
      )}
      ...
      <main className="relative z-10 container mx-auto px-6 md:px-8 py-10 flex-1">
            ...
            <div className="p-8 bg-white/5 rounded-xl border border-white/10 backdrop-blur-lg text-center">
              ...
              <div className="mt-8 pt-6 border-t border-white/10">
                <p className="text-sm text-gray-500">
                  💡 Tip: You can verify your email later from the "My Applications" page
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <p className="text-gray-400 mb-8">
            Choose your role to continue and complete a quick registration.
          </p>
          ...
          {step === "role" && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="p-8 bg-white/5 rounded-xl border border-white/10 backdrop-blur-lg hover:border-purple-500/50 transition-all cursor-pointer group">
                ...
                <h3 className="text-2xl font-semibold text-white mb-2">I am a Student</h3>
                <p className="text-gray-400 mb-6">
                  Apply for scholarships, submit your documents, and receive funds directly to your wallet.
                </p>
                <button
                  onClick={() => onChooseRole("student")}
                  className="w-full px-6 py-3 bg-linear-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 rounded-lg text-white font-semibold transition-all transform hover:scale-105"
                >
                  Continue as Student
                </button>
              </div>
              ...
              <div className="p-8 bg-white/5 rounded-xl border border-white/10 backdrop-blur-lg hover:border-purple-500/50 transition-all cursor-pointer group">
                ...
                <h3 className="text-2xl font-semibold text-white mb-2">I am a Scholarship Provider</h3>
                <p className="text-gray-400 mb-6">
                  Create and manage scholarship pools with full transparency and on-chain disbursement.
                </p>
                <button
                  onClick={() => onChooseRole("provider")}
                  className="w-full px-6 py-3 bg-linear-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 rounded-lg text-white font-semibold transition-all transform hover:scale-105"
                >
          ...
          {step === "student" && (
            <div className="mt-6 p-6 bg-white/5 rounded-xl border border-white/10 backdrop-blur-lg">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-2xl font-semibold text-white">Student Registration</h3>
                <button className="text-sm text-gray-400 hover:text-white transition-colors" onClick={() => setStep("role")}>
              ...
              {!walletAddress && (
                <div className="mb-6 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                  <p className="text-yellow-300 text-sm">
                    ⚠️ Please connect your wallet before completing the registration form.
                  </p>
                </div>
              )}

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Input label="Full Name" name="fullName" value={studentForm.fullName} onChange={handleStudentInput} required />
                <Input label="College Email" name="email" value={studentForm.email} onChange={handleStudentInput} type="email" required />
                <Input label="Institute / University" name="institute" value={studentForm.institute} onChange={handleStudentInput} required />
                <Input label="Program / Major" name="program" value={studentForm.program} onChange={handleStudentInput} required />
                <Input label="Expected Graduation Year" name="graduationYear" value={studentForm.graduationYear} onChange={handleStudentInput} type="number" placeholder="2026" required />
                <div>
                  <label className="block text-sm text-gray-300 mb-1">Connected Wallet</label>
                  <div className="px-4 py-3 bg-white/10 rounded-lg border border-white/20 text-white">
                    {walletAddress ? `${walletAddress.slice(0, 6)}...${walletAddress.slice(-4)}` : "Not connected"}
                  </div>
                </div>
                <div className="md:col-span-2">
                  <label className="block text-sm text-gray-300 mb-1">Supporting Document (Student ID, Transcript, etc.)</label>
                  <input
                    type="file"
                  ...
                  />
                  <p className="text-xs text-gray-500 mt-1">Optional - Upload your student ID or transcript for faster verification</p>
                </div>
              </div>

              <div className="mt-6 flex gap-3">
                <Button onClick={submitStudent} disabled={studentSubmitting || !walletAddress} className="flex-1">
                  {studentSubmitting ? "Submitting..." : !walletAddress ? "Connect Wallet First" : "Complete Registration"}
                </Button>
              </div>
            </div>
          )}
          ...
          {step === "provider" && (
            <div className="mt-6 p-6 bg-white/5 rounded-xl border border-white/10 backdrop-blur-lg">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-2xl font-semibold text-white">Provider Registration</h3>
                <button className="text-sm text-gray-400 hover:text-white transition-colors" onClick={() => setStep("role")}>
                  ← Back to role selection
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Input label="Organization Name" name="organizationName" value={providerForm.organizationName} onChange={handleProviderInput} required />
                <Input label="Work Email" name="email" value={providerForm.email} onChange={handleProviderInput} type="email" required />
                <Input label="Website" name="website" value={providerForm.website} onChange={handleProviderInput} placeholder="https://example.org" />
                <Input label="Contact Person" name="contactPerson" value={providerForm.contactPerson} onChange={handleProviderInput} required />
                <div className="md:col-span-2">
                ...
                <div className="md:col-span-2">
                  <label className="block text-sm text-gray-300 mb-1">Verification Document (Business License, Certificate, etc.)</label>
                  <input
                    type="file"
                    accept=".pdf,.png,.jpg,.jpeg"
                    onChange={(e) => setProviderDoc(e.target.files?.[0] || null)}
                  ...
                  />
                  <p className="text-xs text-gray-500 mt-1">Optional - Upload verification documents for credibility</p>
                </div>
              </div>

              <div className="mt-6 flex gap-3">
                <Button onClick={submitProvider} disabled={providerSubmitting || !walletAddress} className="flex-1">
                  {providerSubmitting ? "Submitting..." : !walletAddress ? "Connect Wallet First" : "Complete Registration"}
                </Button>
              </div>
            </div>
          )}
